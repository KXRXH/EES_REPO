...
def greedilyFindAdjust(net, tick):
    adjStep = 0.1
    adj = 0
    print("here")
    power = p.powerBalance(p.network, *tick)
    print('Preparations are complete')
    while True:
        g = greed(costBalance(power, adj - adjStep),
                  costBalance(power, adj), costBalance(power, adj + adjStep), adjStep)
        if g == 0:
            return adj
        else:
            print(adj)
