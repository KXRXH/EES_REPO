...
class Objects:
    ...
    def get_by_type(self, object_type: str):
        ...
        """
        return max(0, real_weather[object_type][self.eng.act_tick] + random.uniform(-self.HALLWAY, self.HALLWAY))
    ...
    def _power_now_generated(self, obj, itr, online):
        if obj == 'solar' and online:
            return min(self.get_by_type('solar') * self.eng.koaf_solar_gen[itr], MAX_SOLAR)
        if obj == 'wind' and online:
            return min(self.get_by_type('wind') ** 3 * self.eng.koaf_wind_gen[itr], MAX_WIND)
        if obj == 'storage' and online:
            if self.eng.self.eng.delta_storage[itr] < 0:
                return abs(self.eng.self.eng.delta_storage[itr])
        return 0

    def _power_now_consumed(self, obj, itr, online):
        if obj == 'storage' and online:
            if self.eng.delta_storage[itr] > 0:
                return abs(self.eng.delta_storage[itr])
    ...
    def _charge(self, obj, itr):
        if obj == 'storage':
            return self.eng.charge_storage[itr]
        return 0

    def _online(self, obj, itr):
        if obj == 'wind':
            if self.get_by_type('wind') ** 3 * self.eng.koaf_wind_gen[itr] > MAX_POWER_WIND * 100 / MAX_POWER_PERCENT:
                self.eng.online_wind[itr] = False
            if self.get_by_type('wind') ** 3 * self.eng.koaf_wind_gen[
                itr] < MAX_POWER_WIND * MINIMUM_RESUME_PERCENT / MAX_POWER_PERCENT:
                self.eng.online_wind[itr] = True
